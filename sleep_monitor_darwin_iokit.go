...
// isIOKitSupported returns true when IOKit implementation is available
// This is compiled only when 'iokit' build tag is present
func isIOKitSupported() bool {
	return true
}

// newPlatformSleepMonitor creates a macOS-specific sleep monitor
// When compiled with 'iokit' tag, this file will be compiled instead of the polling version
func newPlatformSleepMonitor(useIOKit bool) SleepMonitor {
	// Only IOKit implementation is available in this build
	return newIOKitSleepMonitor()
}

// ============================================================================
// IOKit-based implementation (using mac-sleep-notifier)
// ============================================================================

type iokitSleepMonitor struct {
	events   chan SleepEvent
	done     chan struct{}
	notifier *notifier.Notifier
}

func newIOKitSleepMonitor() SleepMonitor {
	return &iokitSleepMonitor{
...
func (m *iokitSleepMonitor) Start() error {
	slog.Info("Starting macOS sleep monitor using IOKit (mac-sleep-notifier)")
...
func (m *iokitSleepMonitor) Stop() error {
	close(m.done)
	if m.notifier != nil {
		m.notifier.Quit()
	}
...
func (m *iokitSleepMonitor) Events() <-chan SleepEvent {
	return m.events
}
